...
const bundlePattern = /^[a-f0-9]{16}-[a-f0-9]{64}$/;
...
const bundles = readdirSync(__dirname).filter(name => /^[a-f0-9]{16}-[a-f0-9]{64}$/.test(name)).sort();
...
export async function installAdapter(
  ...
  if (latest) await verifyBundle(join(directory, latest), latest.slice(17));
  ...
  const name = `${revision.toString(16).padStart(16, "0")}-${id}`;
  ...
  try {
    if (!latest?.endsWith(`-${id}`)) {
      ...
      try {
        // Fresh destination: even providers that pre-delete overwritten targets
        // cannot remove the previous generation. Do not add overwrite fallback.
        await mutate(() =>
          fs.rename(stage, join(directory, name), { overwrite: false }),
        );
      } catch (error) {
        // Two compatible windows can publish the same revision and bundle.
        if ((error as { code?: string }).code !== "FileExists") throw error;
      }
      await verifyBundle(join(directory, name), id);
    }
    if (!hasLauncher) {
      ...
      try {
        await mutate(() => fs.rename(stagedEntry, entry, { overwrite: false }));
      } catch (error) {
        if ((error as { code?: string }).code !== "FileExists") throw error;
      }
    ...
    // Cleanup owns only these random staging names, including after cancellation.
    await Promise.all(
      cleanup.map((uri) =>
